...
```{r fig-5.2-1, fig.height=30, fig.width=20, out.width='100%'}
...
# Get unique species and calculate number of panels needed
species_list <- unique(hawaiian_means$Species)
n_species <- length(species_list)
n_cols <- 2  # Two columns of panels
n_rows <- ceiling(n_species/n_cols)

# Create multi-panel plot with larger margins
par(mfrow = c(n_rows, n_cols), 
    mar = c(4, 4, 3, 2),  # Increased margins
    oma = c(2, 2, 2, 1))  # Added outer margins

for(species in species_list) {
  species_data <- hawaiian_means %>%
    dplyr::filter(Species == species)
  
  # Plot with larger title margins and increased text size
  plot(NULL, xlim = c(400, 800), ylim = c(0, 100),
       xlab = "", ylab = "",
       main = gsub("_", " ", species),
       cex.main = 1.2,  # Larger title
       cex.axis = 1.1)  # Larger axis labels
  
  # Add grid
  grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")
  
  # Plot each patch
  for(patch in unique(species_data$Patch)) {
    patch_data <- species_data %>%
      dplyr::filter(Patch == patch)
    
    lines(patch_data$Wavelength, patch_data$Mean,
          col = hawaiian_colors[patch],
          lwd = 2.5)  # Thicker lines
  }
  
  # Add legend if it's the first plot
  if(species == species_list[1]) {
    patches <- unique(species_data$Patch)
    legend("topright", legend = patches,
           col = hawaiian_colors[patches],
           lwd = 2.5, cex = 1,  # Larger legend text
           bg = "white", # White background
           box.lwd = 0.5)
  }
  
  # Add axis labels to bottom and left panels only
  if(which(species_list == species) > n_species - n_cols) {
    title(xlab = "Wavelength (nm)", line = 2.5, cex.lab = 1.2)
  }
  if(which(species_list == species) %% n_cols == 1) {
    title(ylab = "Reflectance (%)", line = 2.5, cex.lab = 1.2)
  }
}

# Add overall title
mtext("Hawaiian Fish Spectral Signatures", 
      outer = TRUE, line = 0.5, 
      cex = 1.5, font = 2)
...
- **Key Features**:
  ...
  - Legend on first panel shows patch types
